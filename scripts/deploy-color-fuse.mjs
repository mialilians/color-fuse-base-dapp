...
const source = await readFile(resolve(process.cwd(), "contracts/ColorFuse.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "ColorFuse.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["ColorFuse.sol"]?.ColorFuse;
if (!contract) {
...
console.log(`Deploying ColorFuse from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_COLOR_FUSE_CONTRACT_ADDRESS to this contract address in Vercel.");
